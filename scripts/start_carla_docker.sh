...
if [[ "$OS" == "Darwin" && "$ARCH" == "arm64" ]]; then
    ...
    echo ""
    echo -e "  ${DIM}Official CARLA binaries are built for Linux x86_64 with GPU support.${RESET}"
    echo -e "  ${DIM}Running via Docker + Rosetta 2 emulation is NOT an officially supported${RESET}"
    echo -e "  ${DIM}runtime. It may work for basic connection checks, but CARLA may crash,${RESET}"
    echo -e "  ${DIM}fail to start, or run too slowly for reliable simulator execution.${RESET}"
    echo ""
    echo "  Known limitations:"
    echo "    • No OpenGL / GPU passthrough — software render only"
    echo "    • 20 Hz synchronous ticks are unlikely to be achievable"
    echo "    • Intermittent crashes under memory pressure are common"
    echo "    • Some CARLA API calls may behave unexpectedly under emulation"
    echo ""
    echo "  Recommendation:"
    echo "    Use a Linux or Windows GPU host for reliable data collection."
    echo "    See docs/RUNTIME_PROFILES.md → Decision Tree"
    echo ""
    echo "  Attempting to start anyway. Monitor the container logs:"
    echo "    docker logs -f carla-server"
    echo ""
    echo "  To use a remote Linux server instead (skip Docker entirely):"
